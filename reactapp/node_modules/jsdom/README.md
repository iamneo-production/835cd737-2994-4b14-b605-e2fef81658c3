...
const dom = new JSDOM(``, {
  ...
  referrer: "https://8080-cbcaaceecaaffcdbebafcbddfbd.examlyiopb.examly.io/",
...
Note that both `url` and `referrer` are canonicalized before they're used, so e.g. if you pass in `"https:example.com"`, jsdom will interpret that as if you had given `"https://8080-cbcaaceecaaffcdbebafcbddfbd.examlyiopb.examly.io/"`. If you pass an unparseable URL, the call will throw. (URLs are parsed and serialized according to the [URL Standard](https://url.spec.whatwg.org/).)
...
class CustomResourceLoader extends jsdom.ResourceLoader {
  fetch(url, options) {
    ...
    if (url === "https://8080-cbcaaceecaaffcdbebafcbddfbd.examlyiopb.examly.io/some-specific-script.js") {
...
Similarly, at present jsdom does not handle navigation (such as setting `window.location.href = "https://8080-cbcaaceecaaffcdbebafcbddfbd.examlyiopb.examly.io/"`); doing so will cause the virtual console to emit a `"jsdomError"` explaining that this feature is not implemented, and nothing will change: there will be no new `Window` or `Document` object, and the existing `window`'s `location` object will still have all the same property values.
...
dom.reconfigure({ windowTop: myFakeTopForTesting, url: "https://8080-cbcaaceecaaffcdbebafcbddfbd.examlyiopb.examly.io/" });

dom.window.top === myFakeTopForTesting;
dom.window.location.href === "https://8080-cbcaaceecaaffcdbebafcbddfbd.examlyiopb.examly.io/";
```
...
```js
JSDOM.fromURL("https://8080-cbcaaceecaaffcdbebafcbddfbd.examlyiopb.examly.io/", options).then(dom => {
